...
import projeto.Network;
import projeto.NetworkGraph;
import projeto.recursos.Graph;
import projeto.recursos.IndexMinPQ;
...
public class DijkstraSP {
    ...
    private void relax(FlowEdge e) {
        int v = e.from(), w = e.to();
        if (distTo[w] > distTo[v] + e.capacity()) {
            distTo[w] = distTo[v] + e.capacity();
            edgeTo[w] = e;
    ...
    private boolean check(EdgeWeightedDigraph G, int s) {
        ...
        for (int v = 0; v < G.V(); v++) {
            for (FlowEdge e : G.adj(v)) {
                int w = e.to();
                if (distTo[v] + e.capacity() < distTo[w]) {
